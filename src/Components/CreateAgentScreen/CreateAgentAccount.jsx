...
const CreateAgentAccount = () => {
    ...
    const inputsConfig = [
        { name: 'distributor', label: 'Distributor', inputType: 'dropdown', options: ['Distributor 1', 'Distributor 2', 'Distributor 3'] },
        { name: 'category', label: 'Category', inputType: 'dropdown', options: ['Category 1', 'Category 2', 'Category 3'] },
        { name: 'agentName', label: 'Agency/Agent/Admin name' },
        { name: 'firstName', label: 'First Name' },
        { name: 'middleName', label: 'Middle Name' },
        { name: 'lastName', label: 'Last Name' },
        { name: 'dob', label: 'Date Of Birth', type: 'date' },
        { name: 'gender', label: 'Gender', inputType: 'radio', options: ['Male', 'Female', 'Other'] },
        ...
        { name: 'state', label: 'State', inputType: 'dropdown', options: Object.keys(statesAndDistricts) },
        { name: 'district', label: 'District', inputType: 'dropdown', options: districts },
        { name: 'city', label: 'City', inputType: 'dropdown', options: ['City 1', 'City 2', 'City 3'] },
        { name: 'country', label: 'Country', inputType: 'dropdown', options: ['India'] },
    ...
    const inputsConfig2 = [
        ...
        { name: 'accountType', label: 'Account Type', inputType: 'dropdown', options: ['Savings Account', 'Current Account'] },
        ...
        <div className='w-[1104px] h-[2222px] rounded-[12px] bg-white'>
            <div className='inline-flex items-center p-[10px_61px_10px_10px] gap-[290px] rounded-[4px] bg-[#F4F4F4] ml-[15px] mr-[15px] mt-[12px] mb-[29px]'>
                ...
                <div>
                    ...
                    <label className='text-black font-[600] text-[14px] leading-normal font-open-sans ml-[14px]' htmlFor='checkbox'>
                        Please check the box to create this agent under any Distributor.
                    </label>
                </div>
            </div>
            <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
                {inputsConfig.map(({ name, label, type = 'text', inputType = 'input', options = [] }) => (
                    <AgentsInputs
                        key={name}
                        name={name}
                        label={label}
            ...
            <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
                {inputsConfig2.map(({ name, label, type = 'text', inputType = 'input', options = [] }) => (
                    <AgentsInputs
                        key={name}
                        name={name}
                        label={label}
                        ...
                        onChange={handleChange}
                        value={formValues[name]}
                        error={name === 'password' && formValues.password !== formValues.confirmPassword ? error : undefined}
                    />
                ))}
            ...
            <div className='flex w-[192px] p-[21px_24px] flex-col justify-center items-center gap-[31px] rounded-[8px] bg-[#18213B] shadow-[0px_0px_12.3px_3px_rgba(0,0,0,0.25)] ml-[458px] mr-[502px] mt-[62px] text-white mb-[62px]'>
                    <h1>Basic</h1>
                    <p>₹0.00/ <span>Month</span></p>
                    <div>
                        <input type='checkbox' id='claimItcheckbox' />
                        <label htmlFor='claimItcheckbox'>
                             Claim it
                        </label>
                    </div>
                </div>
            ...
            <div>
                <p className='text-[#FF4545] ml-[58px] mb-[86px] font-open-sans text-[16px] font-normal leading-normal'>Lorem ipsum dolor sit amet consectetur.</p>
            </div>
            {error && (
            ...
            <div className='gap-[23px] inline-flex ml-[569px] mb-[56px]'>
                <button className='w-[231px] p-[12px_18px] rounded-[5px] bg-white border border-[#000] text-black'>Cancel</button>
                <button className='w-[231px] p-[12px_18px] rounded-[5px] bg-[#3E67B0] text-white' onClick={handleCreateAgent}>Create Agent</button>
            </div>
